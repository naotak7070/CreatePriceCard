...
def create_price_cards_from_df_24(df):
    ...
    for i, record in enumerate(records):
        ...
        company = str(record.get('出展者名', '')).strip()
        display_code = str(record.get('display_code', '')).strip()
        jan_code = str(record.get('jan', '')).strip()
        product_name = str(record.get('name', '')).strip()
        msrp = str(record.get('price', '')).strip()
        sales_price = str(record.get('retail_price', '')).strip()
